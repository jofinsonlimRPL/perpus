...
class M_model extends model
{
	...
	public function tampilOderBy($table, $column)
	{
	...
	public function filter_absensi($table, $awal, $akhir)
	{
	    return $this->db->query(
	        "SELECT *
	        FROM " . $table . "
	        JOIN siswa ON " . $table . ".id_absensi_siswa = siswa.id_siswa
	        JOIN ekskul ON siswa.id_siswa_ekskul = ekskul.id_ekskul
	        WHERE " . $table . ".tanggal_laporan BETWEEN ? AND ?",
	        array($awal, $akhir)
	    )->getResult();
	}


	public function filter_absensiBySiswa($table, $awal, $akhir, $nama_siswa)
	{
		return $this->db->query(
			"SELECT *
			FROM " . $table . "
			...
			BETWEEN '" . $awal . "'
			AND '" . $akhir . "'
			AND siswa.nama_siswa = '" . $nama_siswa . "'"
		)->getResult();
	}
...
// 	}
	...
	public function fusionDoubleOrderByWhereAbsen($table1, $table2, $table3, $on1, $on2, $column, $where)
	{
	...
	public function getEkskulWithGuru()
	{
		...
		->join('guru', 'guru.id_guru_ekskul = ekskul.id_ekskul', 'left')
		->orderBy('ekskul.tanggal_ekskul', 'DESC')
		->get()
		->getResult();
	}

	public function getEkskulWithGuruWhere($where)
	{
		return $this->db->table('ekskul')
		->join('guru', 'ekskul.id_ekskul = guru.id_guru_ekskul', 'left')
		->where($where)
		->orderBy('ekskul.tanggal_ekskul', 'DESC')
		->get()
		->getResult();
	...
	public function fusionDouble_where($table1, $table2, $table3, $on1, $on2, $column, $where)
	{
		return $this->db->table($table1)->join($table2, $on1)->join($table3, $on2)->orderBy($column, 'DESC')->getWhere($where)->getResult();
	}

	public function getAbsensiByEkskul($table1, $table2, $table3, $on1, $on2, $column, $id_ekskul)
	{
		$builder = $this->db->table($table1);
		$builder->select("$table1.*, $table2.*, $table3.nama_ekskul");
		$builder->join($table2, "$table1.id_absensi_siswa = $table2.id_siswa");
		$builder->join($table3, "$table2.id_siswa_ekskul = $table3.id_ekskul");
		$builder->where("$table3.id_ekskul", $id_ekskul);
		$builder->orderBy("$table1.$column", 'DESC');
	...
	public function fusionDoubleOderBy($table1, $table2, $table3, $on1, $on2, $column)
	{
		return $this->db->table($table1)->join($table2, $on1)->join($table3, $on2)->orderBy($column, 'DESC')->get()->getResult();
	}

	public function fusionDoubleOrderByLevelGuru($table1, $table2, $table3, $on1, $on2, $column, $where = [])
	{
		$builder = $this->db->table($table1)
		->join($table2, $on1)
		->join($table3, $on2)
	...
	public function fusionDoubleOrderByWhere($table1, $table2, $table3, $on1, $on2, $column, $where)
	{
		return $this->db->table($table1)->join($table2, $on1)->join($table3, $on2)->orderBy($column, 'DESC')->getWhere($where)->getResult();
	}
	...
	public function fusionOderByWhere($table1, $table2, $table3, $on, $on2, $column, $where)
	{
		return $this->db->table($table1)->join($table2, $on)->join($table3, $on2)->orderBy($column, 'DESC')->getWhere($where)->getResult();
	}
	...
	public function ambilDataSiswaEkskul()
	{
        // Mengambil data siswa dan ekskul yang mereka ikuti dengan menghubungkan tabel siswa dan ekskul
        $builder = $this->db->table('siswa'); // Ganti 'siswa' dengan nama tabel siswa Anda
        $builder->select('siswa.id_siswa_ekskul, ekskul.id_ekskul'); // Pilih kolom id_siswa_ekskul dari tabel siswa dan id_ekskul dari tabel ekskul
        $builder->join('ekskul', 'ekskul.id_ekskul = siswa.id_siswa_ekskul'); // Hubungkan tabel siswa dengan tabel ekskul

        $result = $builder->get()->getResult(); // Ambil hasil query

        return $result; // Kembalikan hasilnya dalam bentuk array objek
    }
}